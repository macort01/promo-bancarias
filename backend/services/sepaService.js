...
function stripLeadingZeros(value) {
  return safeTrim(value).replace(/^0+/, '');
}
...
async function loadComercios() {
  ...
  for (const row of rows) {
    const idComercio = safeTrim(row.id_comercio);
    const idBandera = safeTrim(row.id_bandera);
...
async function loadSucursales() {
  ...
  for (const row of rows) {
    const idComercio = safeTrim(row.id_comercio);
    const idSucursal = safeTrim(row.id_sucursal);
    ...
    map.set(key, {
      ...
      direccion: (
        `${pickFirst(row, ['sucursales_calle', 'domicilio', 'direccion', 'calle'])} ${pickFirst(row, ['sucursales_numero'])}`
      ).trim(),
      sucursal_nombre: pickFirst(row, [
        'sucursales_nombre',
        'sucursal_nombre',
        'nom_sucursal',
        'nombre',
  ...
  return new Promise((resolve, reject) => {
    ...
    fs.createReadStream(SEPA_CSV_PATH)
      ...
      .on('data', (row) => {
        ...
        if (!sucursal) {
          const idSucursalNoZeros = stripLeadingZeros(idSucursal);

          const match = Array.from(sucursalesMap.values()).find((s) => {
            return (
              safeTrim(s.id_comercio) === idComercio &&
              stripLeadingZeros(s.id_sucursal) === idSucursalNoZeros
            );
          });
        ...
        const item = {
          ...
          sucursal: sucursal?.sucursal_nombre || sucursal?.direccion || `Sucursal ${idSucursal}`,
    ...
    .map(([key, items]) => ({
      ...
      comercios: items
        .map((item) => ({
          ...
          ubicacion: [item.localidad, item.provincia].filter(Boolean).join(', ') || 'Ubicación no disponible',
