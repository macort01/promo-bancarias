...
function parseCsvBuffer(buffer) {
  return new Promise((resolve, reject) => {
    ...
    Readable.from(buffer)
      .pipe(csv())
      .on('data', (row) => {
        if (results.length >= MAX_ROWS_TO_SCAN) {
          return;
        }
        results.push({
...
async function refreshCache() {
  ...
  const zipResponse = await axios.get(zipUrl, { timeout: 120000, responseType: 'arraybuffer', maxContentLength: 1024 * 1024 * 1024 });
...
async function searchProducts({ query, provincia, limit = 20 }) {
  ...
  const products = Array.from(grouped.entries()).map(([key, items], index) => ({
    id: key,
